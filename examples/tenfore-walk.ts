...
const gait = walkCycle({
  stance: STANCE,
  reach: REACH,
  // The beak points left, so the bird walks left. Without this it keeps a
...
tenforeWalk.part('body').animate(
  bodyBob({ stance: STANCE, legLength: SEGMENTS[0] + SEGMENTS[1], reach: REACH }),
);
tenforeWalk.part('neck').animate(sway(2, { stance: STANCE }));
// The head counter-rotates so it stays level while the neck works under it.
tenforeWalk.part('head').animate(sway(-1.5, { stance: STANCE }));
