/**
 * The agent feedback loop. `cli.ts` is the entry; the commands live here.
 *
...
import { encodePng, resvgFont } from './raster.ts';
...
async function typecheckScene(file: string): Promise<string[]> {
  ...
  try {
    ...
    return ['heron: typescript is not installed, so the scene was not typechecked (pnpm add -D typescript)'];
  }
  ...
  // Only the scene and what it imports. The tsconfig's whole `include` set was
  // the root list before, which typechecked every example in the repository on
  // every `check` and cost several seconds per run for nothing the scene needed.
  //
  // Incremental, with the build state kept beside the tsconfig it belongs to:
  // the library's own declarations are most of the work, and an agent runs
  // `check` many times against the same library. Warm runs cost about half.
  const options = {
    ...parsed.options,
    noEmit: true,
    incremental: true,
    tsBuildInfoFile: join(dirname(configFile), 'node_modules', '.cache', 'heron', 'check.tsbuildinfo'),
  };
  const program = ts.createIncrementalProgram({ rootNames: [resolve(file)], options });
  const diagnostics = [
  ...
  try {
    mkdirSync(dirname(options.tsBuildInfoFile), { recursive: true });
    program.emit();
  } catch {
    // A read-only checkout still typechecks; it only stays cold.
  }
...
function toPng(svg: string, width: number): Buffer {
  const script = `
    ...
    const png = new Resvg(svg, {
      ...
      font: JSON.parse(process.argv[5]),
    }).render().asPng();
  ...
  try {
    ...
    const run = spawnSync(process.execPath, ['-e', script, svgFile, pngFile, String(width), resvgPath(), JSON.stringify(resvgFont())], {
 ...
 *
 * The sheets render concurrently on the thread pool: `renderAsync` releases the
 * JavaScript thread, and nothing about one sheet depends on another.
 */
function writePngBatch(items: Array<{ svg: string; file: string; width: number }>): void {
  if (!items.length) return;
  const script = `
    const { readFileSync, writeFileSync } = require('node:fs');
    const { renderAsync } = require(process.argv[2]);
    const { tasks, font } = JSON.parse(readFileSync(process.argv[1], 'utf8'));
    Promise.all(tasks.map(async (task) => {
      const svg = readFileSync(task.svg, 'utf8');
      const image = await renderAsync(svg, {
        fitTo: { mode: 'width', value: task.width }, background: 'white', font,
      });
      writeFileSync(task.png, image.asPng());
  ...
  try {
    ...
    writeFileSync(manifest, JSON.stringify({ tasks, font: resvgFont() }));
...
/** A flag whose value must be one of a few words, named back when it is not. */
function oneOf<const T extends readonly string[]>(flag: string, value: string, allowed: T): T[number] {
  if (!allowed.includes(value)) {
    throw new Error(`heron: ${flag} must be one of ${allowed.join(', ')}, got "${value}"`);
  }
  return value;
}
...
const USAGE = `heron - author character animation and compile it for SVG, Lottie or video
  ...
  heron match    <scene.ts> <reference.png> [-o overlay.png] [-t 0] [--threshold 0.22] [--json]
  ...
  heron frames   <scene.ts> [-o frames] [--fps 30] [--per-sheet 12] [--cols 4]
                 [--range <cue>|<a..b>] [--cue <name>] [--cell 260] [--svg]
  heron motion   <scene.ts> --part <path[@x,y]> [--part ...] [--compare <path>] [-n 24]
                 [--range <cue>|<a..b>] [--cue <name>] [--cues] [--zoom] [-o motion.png] [--svg] [--no-json]
  heron variants <scene.ts> [-t 0.5] [-n 24] [--strip 3] [--cue <name>] [--range <a..b>]
                 [--motion <path>] [--compare <path>] [--zoom] [--only axis=value] [--cols 3]
                 [-o variants.png] [--svg] [--no-json]
  heron inspect  <scene.ts> [-t 0.4] [--json]
  heron curves   <scene.ts> [-t 0,0.25,0.5,0.75,1] [-n 240] [--json]
  heron check    <scene.ts> [--fps 60] [--reference artwork.png] [-t 0] [--threshold 0.22] [--json]
  heron lint     <scene.ts> [--fps 60] [--json]
  ...
  heron video    <scene.ts> [-o out.mp4] [-w 1280] [--fps 30] [--audio soundtrack.wav]
                 [--codec h264|h265|vp9] [--crf 18] [--background white]
...
export async function main(): Promise<void> {
  ...
  if (cmd === 'trace') {
    ...
    const res = trace(file, {
      ...
      ribbons: args.ribbons ? oneOf('--ribbons', String(args.ribbons), ['taper', 'all', 'none']) : undefined,
